...
func GetDefaultCollector() *colly.Collector {
	...
	mdb2.FindAll(dbName, "account", bson.M{}, bson.M{}, &cookieStrLi)
...
func setDefaultCallback(c *colly.Collector) {
	...
	c.OnError(func(r *colly.Response, e error) {
		if r.StatusCode == 302 || r.StatusCode == 403 {
			mdb2.Update(dbName, "account", bson.M{"_id": r.Ctx.Get("_id")}, bson.M{"$set": bson.M{"status": "error"}})
